...
class delivery_method(osv.osv):
    ...
    _description = " Delivery Method or Category"
    
    def _get_company_code(self, cr, user, context=None):
        return [('grid', 'Price Grid')]
    
    _columns = {
        'name': fields.char('Delivery Method', size=24, required=True),
        ...
        'delivery_method_delivery_carrier': fields.many2many('delivery.carrier', 'delivery_method_delivery_carrier', 'del_id', 'carier_id', 'Delivery Carrier'),
        'ship_account_id': fields.property(type='many2one',relation='account.account', string="Cost Account",view_load=True),
     }
...
class delivery_carrier(osv.osv):
    ...
    _description = "Delivery Carrier Services"
    ...
    def get_price(self, cr, uid, ids, field_name, arg=None, context=None):
        ...
        for carrier in self.browse(cr, uid, ids, context=context):
            ...
            price=False
            if sale_order_id:
              order = sale_obj.browse(cr, uid, sale_order_id, context=context)
            ...
            if purchase_order_id:
              order = purchase_obj.browse(cr, uid, purchase_order_id, context=context)
              
              destination=order.dest_address_id.id or False
              if not destination:
                  destination = order.warehouse_id.partner_id.id or False
              carrier_grid=self.grid_get(cr,uid,[carrier.id],destination,context)
              if carrier_grid:
                  price=grid_obj.get_price(cr, uid, carrier_grid, order, time.strftime('%Y-%m-%d'), context)
              else:
                  price = 0.0
    ...
    _columns = {
        'name': fields.char('Delivery Service', size=64, required=True),
...
#        'product_id': fields.many2one('product.product', 'Related Delivery Product',
#                                        help="This is the related Product added to Sale Line with Delivery Service Cost",
#                                        required=True),
        'grids_id': fields.one2many('delivery.grid', 'carrier_id', 'Delivery Price Grids'),
...
#        'delivery_method': fields.many2one('delivery.method', 'Delivery Method or Category'),
    ...
    def search(self, cr, uid, args, offset=0, limit=None, order=None,
        ...
        if context.get('delivery_method', False):
            ...
            for rec in method_obj:
                ...
                ids = [x.id for x in rec.delivery_method_delivery_carrier]
                args += [('id', 'in', ids)]
...
class delivery_grid(osv.osv):
    ...
    _columns = {
        ...
        'carrier_id': fields.many2one('delivery.carrier', 'Delivery Mode', required=True, ondelete='cascade'),
    ...
    def get_price_sale(self, cr, uid, id, order, dt, context=None):
        total = 0
        weight = 0
        volume = 0
    ...
    def get_price_purchase(self, cr, uid, id, order, dt, context=None):
        ...
        for line in order.order_line:
            if not line.product_id:
                continue
            q = product_uom_obj._compute_qty(cr, uid, line.product_uom.id, line.product_qty, line.product_id.uom_id.id)
            total += line.price_subtotal or 0.0
            weight += (line.product_id.weight or 0.0) * q
            volume += (line.product_id.volume or 0.0) * q
